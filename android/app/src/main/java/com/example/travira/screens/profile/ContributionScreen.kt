...
fun ContributionScreen(
    ...
    LaunchedEffect(Unit) {
        ...
        try {
            ...
            val res = RetrofitInstance.placeApi.getMyPlaces("Bearer $token")
            places = res.places
        } catch (e: Exception) {
            error = e.message
        } finally {
            ...
            when {
                ...
                places.isEmpty() -> {
                    Column(
                        ...
                        Text("You haven't added any places yet", fontSize = 16.sp, color = Color.Gray)
                        if (error != null) {
                    ...
                    LazyColumn(
                        ...
                        items(places, key = { it._id.ifBlank { it.name } }) { place ->
                            AppCard(
                                ...
                                statusLabel = place.approvalStatus
                            )
                        }
